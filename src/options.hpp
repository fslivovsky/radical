...
// 'L' column determines which options do not support LRAT (disabled with
// '--plainlrat')

#define OPTIONS \
\
/*      NAME         DEFAULT, LO, HI,O,P,L,R, USAGE */ \
\
OPTION( arena,             1,  0,  1,0,0,0,1, "allocate clauses in arena") \
OPTION( arenacompact,      1,  0,  1,0,0,0,1, "keep clauses compact") \
...
OPTION( bump,              1,  0,  1,0,0,0,1, "bump variables") \
OPTION( bumpreason,        1,  0,  1,0,0,0,1, "bump reason literals too") \
OPTION( bumpreasondepth,   1,  1,  3,0,0,0,1, "bump reason depth") \
OPTION( check,             0,  0,  1,0,0,0,0, "enable internal checking") \
OPTION( checkassumptions,  1,  0,  1,0,0,0,0, "check assumptions satisfied") \
OPTION( checkconstraint,   1,  0,  1,0,0,0,0, "check constraint satisfied") \
OPTION( checkfailed,       1,  0,  1,0,0,0,0, "check failed literals form core") \
OPTION( checkfrozen,       0,  0,  1,0,0,0,0, "check all frozen semantics") \
...
OPTION( elimboundmax,     16, -1,2e6,1,0,0,1, "maximum elimination bound") \
OPTION( elimboundmin,      0, -1,2e6,0,0,0,1, "minimum elimination bound") \
OPTION( elimclslim,      1e2,  2,2e9,2,0,0,1, "resolvent size limit") \
OPTION( elimequivs,        1,  0,  1,0,0,0,1, "find equivalence gates") \
OPTION( elimineff,       1e7,  0,2e9,1,0,0,1, "minimum elimination efficiency") \
OPTION( elimint,         2e3,  1,2e9,0,0,0,1, "elimination interval") \
OPTION( elimites,          1,  0,  1,0,0,0,1, "find if-then-else gates") \
OPTION( elimlimited,       1,  0,  1,0,0,0,1, "limit resolutions") \
OPTION( elimocclim,      1e2,  0,2e9,2,0,0,1, "occurrence limit") \
OPTION( elimprod,          1,  0,1e4,0,0,0,1, "elim score product weight") \
OPTION( elimreleff,      1e3,  1,1e5,1,0,0,1, "relative efficiency per mille") \
OPTION( elimrounds,        2,  1,512,1,0,0,1, "usual number of rounds") \
OPTION( elimsubst,         1,  0,  1,0,0,0,1, "elimination by substitution") \
OPTION( elimsum,           1,  0,1e4,0,0,0,1, "elimination score sum weight") \
OPTION( elimxorlim,        5,  2, 27,1,0,0,1, "maximum XOR size") \
OPTION( elimxors,          1,  0,  1,0,0,0,1, "find XOR gates") \
OPTION( emagluefast,      33,  1,2e9,0,0,0,1, "window fast glue") \
OPTION( emaglueslow,     1e5,  1,2e9,0,0,0,1, "window slow glue") \
OPTION( emajump,         1e5,  1,2e9,0,0,0,1, "window back-jump level") \
OPTION( emalevel,        1e5,  1,2e9,0,0,0,1, "window back-track level") \
OPTION( emasize,         1e5,  1,2e9,0,0,0,1, "window learned clause size") \
OPTION( ematrailfast,    1e2,  1,2e9,0,0,0,1, "window fast trail") \
OPTION( ematrailslow,    1e5,  1,2e9,0,0,0,1, "window slow trail") \
OPTION( flush,             0,  0,  1,0,0,0,1, "flush redundant clauses") \
OPTION( flushfactor,       3,  1,1e3,0,0,0,1, "interval increase") \
OPTION( flushint,        1e5,  1,2e9,0,0,0,1, "initial limit") \
OPTION( forcephase,        0,  0,  1,0,0,0,1, "always use initial phase") \
OPTION( inprocessing,      1,  0,  1,0,0,0,1, "enable inprocessing") \
OPTION( instantiate,       0,  0,  1,0,1,1,1, "variable instantiation") \
OPTION( instantiateclslim, 3,  2,2e9,0,0,0,1, "minimum clause size") \
OPTION( instantiateocclim, 1,  1,2e9,2,0,0,1, "maximum occurrence limit") \
...
OPTION( minimize,          1,  0,  1,0,0,1,1, "minimize learned clauses") \
OPTION( minimizedepth,   1e3,  0,1e3,0,0,0,1, "minimization depth") \
OPTION( phase,             1,  0,  1,0,0,0,1, "initial phase") \
OPTION( probe,             1,  0,  1,0,1,1,1, "failed literal probing" ) \
...
OPTION( probereleff,      20,  1,1e5,1,0,0,1, "relative efficiency per mille") \
OPTION( proberounds,       1,  1, 16,1,0,0,1, "probing rounds" ) \
OPTION( profile,           2,  0,  4,0,0,0,0, "profiling level") \
QUTOPT( quiet,             0,  0,  1,0,0,0,0, "disable all messages") \
OPTION( radixsortlim,    800,  0,2e9,0,0,0,1, "radix sort limit") \
OPTION( realtime,          0,  0,  1,0,0,0,0, "real instead of process time") \
OPTION( reduce,            1,  0,  1,0,0,0,1, "reduce useless clauses") \
OPTION( reduceint,       300, 10,1e6,0,0,0,1, "reduce interval") \
OPTION( reducetarget,     75, 10,1e2,0,0,0,1, "reduce fraction in percent") \
OPTION( reducetier1glue,   2,  1,2e9,0,0,0,1, "glue of kept learned clauses") \
OPTION( reducetier2glue,   6,  1,2e9,0,0,0,1, "glue of tier two clauses") \
OPTION( reluctant,      1024,  0,2e9,0,0,0,1, "reluctant doubling period") \
OPTION( reluctantmax,1048576,  0,2e9,0,0,0,1, "reluctant doubling period") \
OPTION( rephase,           1,  0,  1,0,0,0,1, "enable resetting phase") \
OPTION( rephaseint,      1e3,  1,2e9,0,0,0,1, "rephase interval") \
OPTION( report,reportdefault,  0,  1,0,0,0,1, "enable reporting") \
OPTION( reportall,         0,  0,  1,0,0,0,1, "report even if not successful") \
OPTION( reportsolve,       0,  0,  1,0,0,0,1, "use solving not process time") \
OPTION( restart,           1,  0,  1,0,0,0,1, "enable restarts") \
OPTION( restartint,        2,  1,2e9,0,0,0,1, "restart interval") \
OPTION( restartmargin,    10,  0,1e2,0,0,0,1, "slow fast margin in percent") \
OPTION( restartreusetrail, 1,  0,  1,0,0,0,1, "enable trail reuse") \
OPTION( restoreall,        0,  0,  2,0,0,0,1, "restore all clauses (2=really)") \
OPTION( restoreflush,      0,  0,  1,0,0,0,1, "remove satisfied clauses") \
OPTION( reverse,           0,  0,  1,0,0,0,1, "reverse variable ordering") \
...
OPTION( shrink,            3,  0,  3,0,0,1,1, "shrink conflict clause") \
...
OPTION( shufflerandom,     0,  0,  1,0,0,0,1, "not reverse but random") \
OPTION( shufflescores,     1,  0,  1,0,0,0,1, "shuffle variable scores") \
OPTION( stabilize,         1,  0,  1,0,0,0,1, "enable stabilizing phases") \
OPTION( stabilizefactor, 200,101,2e9,0,0,0,1, "phase increase in percent") \
OPTION( stabilizeint,    1e3,  1,2e9,0,0,0,1, "stabilizing interval") \
OPTION( stabilizemaxint, 2e9,  1,2e9,0,0,0,1, "maximum stabilizing phase") \
OPTION( stabilizeonly,     0,  0,  1,0,0,0,1, "only stabilizing phases") \
OPTION( subsume,           1,  0,  1,0,1,1,1, "enable clause subsumption") \
OPTION( subsumebinlim,   1e4,  0,2e9,1,0,0,1, "watch list length limit") \
OPTION( subsumeclslim,   1e2,  0,2e9,2,0,0,1, "clause length limit") \
OPTION( subsumeint,      1e4,  1,2e9,0,0,0,1, "subsume interval") \
OPTION( subsumelimited,    1,  0,  1,0,0,0,1, "limit subsumption checks") \
OPTION( subsumemaxeff,   1e8,  0,2e9,1,0,0,1, "maximum subsuming efficiency") \
OPTION( subsumemineff,   1e6,  0,2e9,1,0,0,1, "minimum subsuming efficiency") \
OPTION( subsumeocclim,   1e2,  0,2e9,1,0,0,1, "watch list length limit") \
OPTION( subsumereleff,   1e3,  1,1e5,1,0,0,1, "relative efficiency per mille") \
...
OPTION( ternaryocclim,   1e2,  1,2e9,2,0,0,1, "ternary occurrence limit") \
OPTION( ternaryreleff,    10,  1,1e5,1,0,0,1, "relative efficiency per mille") \
OPTION( ternaryrounds,     2,  1, 16,1,0,0,1, "maximum ternary rounds") \
OPTION( transred,          1,  0,  1,0,1,1,1, "transitive reduction of BIG") \
OPTION( transredmaxeff,  1e8,  0,2e9,1,0,0,1, "maximum efficiency") \
OPTION( transredmineff,  1e6,  0,2e9,1,0,0,1, "minimum efficiency") \
OPTION( transredreleff,  1e2,  1,1e5,1,0,0,1, "relative efficiency per mille") \
QUTOPT( verbose,           0,  0,  3,0,0,0,0, "more verbose messages") \
OPTION( vivify,            1,  0,  1,0,1,1,1, "vivification") \
...
OPTION( vivifyredeff,     75,  0,1e3,1,0,0,1, "redundant efficiency per mille") \
OPTION( vivifyreleff,     20,  1,1e5,1,0,0,1, "relative efficiency per mille") \
OPTION( walk,              1,  0,  1,0,0,0,1, "enable random walks") \
OPTION( walkmaxeff,      1e7,  0,2e9,1,0,0,1, "maximum efficiency") \
OPTION( walkmineff,      1e5,  0,1e7,1,0,0,1, "minimum efficiency") \
OPTION( walknonstable,     1,  0,  1,0,0,0,1, "walk in non-stabilizing phase") \
OPTION( walkredundant,     0,  0,  1,0,0,0,1, "walk redundant clauses too") \
OPTION( walkreleff,       20,  1,1e5,1,0,0,1, "relative efficiency per mille") \
...
#define OPTION(N,V,L,H,O,P,T,R,D) 1 +
OPTIONS
#undef OPTION
+ 0;
...
# undef OPTION
  ...
  static bool is_non_lrat_option (const char * name);
